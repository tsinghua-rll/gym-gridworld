...
COLORS = {0:[0.0,0.0,0.0],
          1:[200/255,200/255,200/255],
          2:[95/255,95/255,95/255],
          3:[0.0,1.0,0.0],
          4:[1.0,0.0,0.0],
          6:[1.0,0.0,1.0],
          7:[1.0,1.0,0.0]}
...
class GridworldRandomEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.obs_shape = self.start_grid_map.reshape(-1).shape  # observation space shape
        self.image_shape = [128, 128, 3]
        self.observation_space = spaces.Box(low=0, high=100, shape=self.obs_shape, dtype=np.float32)
        ...
        GridworldRandomEnv.num_env += 1
        self.this_fig_num = GridworldRandomEnv.num_env
    ...
    def step(self, action):
        ...
        if nxt_agent_state[0] == self.agent_target_state[0] and nxt_agent_state[1] == self.agent_target_state[1] :
            ...
            if self.restart_once_done:
                ...
                info['success'] = True
                return (target_observation, reward, True, info)
        else:
            info['success'] = True
            return (self.observation, reward, False, info)
    ...
    def reset(self):
        ...
        while True:
            sp = np.random.randint(0, size,(2,))
            if self.change_start_state(sp):
                break
        while True:
            tg = np.random.randint(0, size,(2,))
            if not (sp == tg).any() and self.change_target_state(tg):
                break
    ...
    def _gridmap_to_image(self, grid_map, obs_shape=None):
        if obs_shape is None:
            obs_shape = self.image_shape
        observation = np.zeros(obs_shape, dtype=np.float32)
        gs0 = int(observation.shape[0]/grid_map.shape[0])
        gs1 = int(observation.shape[1]/grid_map.shape[1])
        for i in range(grid_map.shape[0]):
    ...
    def _render(self, mode='human', close=False):
        if self.verbose == False:
            return
        img = self._gridmap_to_image(self.current_grid_map)
    ...
    def change_start_state(self, sp):
        ...
        if self.start_grid_map[sp[0], sp[1]] in [1, 2]:
            return False
        else:
    ...
    def change_target_state(self, tg):
        if self.start_grid_map[tg[0], tg[1]] in [1, 2]:
            return False
        else:
...
class SimpleGridworldRandomEnv(gym.Env):
    metadata = {'render.modes': ['human']}
    num_env = 0
    def __init__(self):
        self._seed = 0
        ...
        SimpleGridworldRandomEnv.num_env += 1
        self.this_fig_num = SimpleGridworldRandomEnv.num_env
        if self.verbose == True:
            self.fig = plt.figure(self.this_fig_num)
            plt.show(block=False)
            plt.axis('off')
...
class SimpleGridworldEnv(gym.Env):
    ...
    def __init__(self):
        ...
        SimpleGridworldEnv.num_env += 1
        self.this_fig_num = SimpleGridworldEnv.num_env
        if self.verbose == True:
            self.fig = plt.figure(self.this_fig_num)
            plt.show(block=False)
            plt.axis('off')
...
class SimpleGridworldRGBEnv(gym.Env):
    metadata = {'render.modes': ['human']}
    num_env = 0
    def __init__(self):
        self._seed = 0
        ...
        GridworldRGBEnv.num_env += 1
        self.this_fig_num = GridworldRGBEnv.num_env
        if self.verbose == True:
            self.fig = plt.figure(self.this_fig_num)
            plt.show(block=False)
            plt.axis('off')
...
class GridworldAAEnv(gym.Env):
    metadata = {'render.modes': ['human']}
    num_env = 0
    def __init__(self, aa_num=20):
        self._seed = 0
        self.aa_num = aa_num
        # generate aa action and action position dictionary
        action_extra = []
        action_pos_dict_extra = {}
        for i in range(self.aa_num):
            index = 4 * (i + 1)
            action_extra.extend([index + 1, index + 2, index + 3, index + 4])
            action_pos_dict_extra[index + 1] = [-1, 0]
            action_pos_dict_extra[index + 2] = [1, 0]
            action_pos_dict_extra[index + 3] = [0, -1]
            action_pos_dict_extra[index + 4] = [0, 1]

        self.actions = [0, 1, 2, 3, 4]
        self.actions.extend(action_extra)
     
        # self.inv_actions = [0, 2, 1, 4, 3]
        self.action_space = spaces.Discrete(5+4*self.aa_num)
        self.action_pos_dict = {
            0: [0,0],
            1:[-1, 0],
            2:[1,0],
        ...
        self.action_pos_dict.update(action_pos_dict_extra)
        self.max_iterations = 1000
        self.cur_iter = 0
        ...
        GridworldAAEnv.num_env += 1
        self.this_fig_num = GridworldAAEnv.num_env
        if self.verbose == True:
            self.fig = plt.figure(self.this_fig_num)
            plt.show(block=False)
            plt.axis('off')
